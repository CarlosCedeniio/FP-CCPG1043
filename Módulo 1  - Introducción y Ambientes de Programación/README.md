# Módulo 1 - Introducción y Ambientes de Programación

Este módulo cubre los conceptos fundamentales para iniciarse en la programación con Python. Se presentan las políticas del curso, el entorno de desarrollo, y una introducción al lenguaje Python: su sintaxis básica, tipos de datos, y primeras instrucciones.

## Contenido

| Archivo | Descripción |
|---------|-------------|
| [Políticas del curso](Politicas%20del%20curso.html) | Información general del curso, políticas de asistencia, calificaciones y uso de LLMs |
| [1.1 Introducción a Python](1.1%20Introducción%20a%20python.html) | Introducción al lenguaje Python: historia, características y primeros pasos |
| [1.2 Conceptos esenciales](1.2%20Conceptos%20esenciales.html) | Conceptos esenciales de programación: variables, expresiones, instrucciones y flujo de ejecución |

## Recursos adicionales

- [Página oficial de la materia](http://programacion.espol.edu.ec/)
- [Ejercicios y exámenes resueltos](http://blog.espol.edu.ec/ccpg1001/)
- [Ejercicios Python3 - Universidad de Valencia](https://www.mclibre.org/consultar/python/)
- [CS50 Python Harvard <- Muy recomendado](https://cs50.harvard.edu/python/weeks/0/)

